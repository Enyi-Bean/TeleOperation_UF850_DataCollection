...
class UF850WebXRControlCorrect:
    ...
    def __init__(self, robot_ip=None):
        ...
        self.POSITION_SCALE = 3.0    # 位置缩放因子
    ...
    def compute_target_pose(self, vr_x, vr_y, vr_z, vr_qx, vr_qy, vr_qz, vr_qw):
        ...
        # 工作空间限制 - 暂时注释掉以测试无限制效果
        # ⚠️ 警告：注释掉这些限制可能导致机械臂移动到危险位置！
        # 请确保周围没有障碍物，并准备好紧急停止！
        ...
        # 添加调试输出，显示是否超出原限制
        if (target_pos[0] < config.WORKSPACE_LIMITS['x'][0] or
            target_pos[0] > config.WORKSPACE_LIMITS['x'][1]):
            print(f"⚠️ X轴超出限制: {target_pos[0]:.1f}mm (限制: {config.WORKSPACE_LIMITS['x']})")
        if (target_pos[1] < config.WORKSPACE_LIMITS['y'][0] or
            target_pos[1] > config.WORKSPACE_LIMITS['y'][1]):
            print(f"⚠️ Y轴超出限制: {target_pos[1]:.1f}mm (限制: {config.WORKSPACE_LIMITS['y']})")
        if (target_pos[2] < config.WORKSPACE_LIMITS['z'][0] or
            target_pos[2] > config.WORKSPACE_LIMITS['z'][1]):
            print(f"⚠️ Z轴超出限制: {target_pos[2]:.1f}mm (限制: {config.WORKSPACE_LIMITS['z']})")
    ...
    async def run_server(self):
        """运行WebSocket服务器"""
        port = 8765
        print(f"WebSocket服务器启动: 端口 {port}")
