#!/usr/bin/env python3
"""
UF850 WebXR 遥操作控制（镜像+旋转版本）
基于robot_control_with_rotation.py，修改为镜像模式
适用于面对面操作机械臂，且需要精确姿态控制的任务
"""
...
class UF850WebXRControlMirrorWithRotation:
    """UF850 WebXR 遥操作控制器（镜像+旋转版本）"""
    ...
    def __init__(self, robot_ip=None):
        ...
        print("\n" + "="*60)
        print("坐标系配置（镜像模式 + 旋转控制）:")
        print("  Quest 3 WebXR: X=右, Y=上, Z=后(朝向用户)")
        print("  UF850 机械臂: X=前, Y=左, Z=上")
        print("  位置映射(面对面镜像):")
        print("    VR Z(后) → Robot X(前) [镜像不取反]")
        print("    VR X(右) → Robot Y(左) [镜像不取反]")
    ...
    def vr_rotation_to_robot(self, qx, qy, qz, qw):
        ...
        # 镜像模式的坐标系变换矩阵
        # 与位置变换对应的镜像映射
        T_vr_to_robot_mirror = np.array([
            [ 0,  0,  1],  # Robot X = VR Z (镜像模式)
            [ 1,  0,  0],  # Robot Y = VR X (镜像模式)
            [ 0,  1,  0]   # Robot Z = VR Y
        ])

        # 变换旋转矩阵到机械臂坐标系
        R_robot = T_vr_to_robot_mirror @ R_vr @ T_vr_to_robot_mirror.T

        # 镜像模式下，某些旋转方向需要反转
        # 特别是Roll和Pitch在面对面操作时
        rpy = self.rotation_matrix_to_rpy(R_robot)
        rpy[0] = -rpy[0]  # Roll镜像反转
        rpy[1] = -rpy[1]  # Pitch镜像反转
        # Yaw保持不变（绕Z轴旋转在镜像下方向一致）

        R_robot_mirrored = self.rpy_to_rotation_matrix(rpy[0], rpy[1], rpy[2])

        return R_robot_mirrored

    async def initialize_robot(self):
        """初始化机械臂"""
        print("初始化UF850机械臂（镜像+旋转控制）...")
    ...
    async def handle_controller_data(self, websocket):
        ...
        print(f"Quest 3 已连接: {client_ip} (镜像+旋转控制)")
            ...
            async for message in websocket:
                try:
                    ...
                    if self.frame_count % 50 == 0:
                        ...
                        print(f"\n[Frame {self.frame_count}] 镜像+旋转模式")
    ...
    async def run_server(self):
        """运行WebSocket服务器"""
        port = 8765
        print(f"WebSocket服务器启动（镜像+旋转控制）: 端口 {port}")

        if not self.is_calibrated:
            print("\n等待标定...")
            print("将右手柄移到舒适位置，按住Trigger键标定")
            print("提示: 镜像模式适合面对面操作，旋转也会镜像\n")
...
async def main():
    ...
    controller = UF850WebXRControlMirrorWithRotation(robot_ip)
