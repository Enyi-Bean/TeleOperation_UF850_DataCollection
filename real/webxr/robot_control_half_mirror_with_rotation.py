#!/usr/bin/env python3
"""
UF850 WebXR 遥操作控制（半镜像+旋转版本）
基于robot_control_mirror_with_rotation.py，修改为前后不镜像，左右镜像
适用于面对面操作机械臂，前后动作一致，左右镜像
"""
...
class UF850WebXRControlHalfMirrorWithRotation:
    """UF850 WebXR 遥操作控制器（半镜像+旋转版本）"""
    ...
    def vr_to_robot_position(self, x, y, z):
        ...
        """
        # 半镜像坐标映射
        robot_x = -z  # VR的后(+Z) → Robot的前(+X)，前后不镜像，需要取反
        robot_y = x   # VR的右(+X) → Robot的左(+Y)，左右镜像，不取反
    ...
    def vr_rotation_to_robot(self, qx, qy, qz, qw):
        ...
        T_vr_to_robot_half_mirror = np.array([
            [ 0,  0, -1],  # Robot X = -VR Z (前后不镜像)
            [ 1,  0,  0],  # Robot Y = VR X (左右镜像)
            [ 0,  1,  0]   # Robot Z = VR Y
        ])
        ...
        # 半镜像模式下的旋转调整
        # Roll需要镜像反转（绕X轴，影响左右倾斜）
        # Pitch不需要反转（绕Y轴，影响前后俯仰）
        # Yaw需要反转（绕Z轴，影响左右转向）
        rpy = self.rotation_matrix_to_rpy(R_robot)
        rpy[0] = -rpy[0]  # Roll镜像反转（左右倾斜镜像）
        # rpy[1] 保持不变  # Pitch不反转（前后俯仰一致）
        rpy[2] = -rpy[2]  # Yaw镜像反转（左右转向镜像）
    ...
    async def handle_controller_data(self, websocket):
        ...
        print(f"Quest 3 已连接: {client_ip} (半镜像+旋转控制)")
            ...
            async for message in websocket:
                try:
                    ...
                    if self.frame_count % 50 == 0:
                        ...
                        print(f"\n[Frame {self.frame_count}] 半镜像+旋转模式")
    ...
    async def run_server(self):
        """运行WebSocket服务器"""
        port = 8765
        print(f"WebSocket服务器启动（半镜像+旋转控制）: 端口 {port}")

        if not self.is_calibrated:
            print("\n等待标定...")
            print("将右手柄移到舒适位置，按住Trigger键标定")
            print("提示: 半镜像模式 - 前后动作一致，左右镜像\n")
...
async def main():
    ...
    controller = UF850WebXRControlHalfMirrorWithRotation(robot_ip)
